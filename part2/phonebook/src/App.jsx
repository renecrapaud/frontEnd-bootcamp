...
const App = () => {
  ...
  const bookFiltered =
    searchField == ""
      ? persons
      : persons.filter((person) => person.name.includes(searchField));
  ...
  const addName = (event) => {
    ...
    if (personReg && personReg.name === newName) {
      if (
        ...
        phoneBookEntry.update(personReg.id, nameObject).then((response) => {
          setPersons(
            persons.map((person) =>
              person.id !== personReg.id ? person : nameObject,
            ),
          );
      ...
      phoneBookEntry.create(nameObject).then((response) => {
  ...
  useEffect(() => {
    phoneBookEntry.getAll().then((response) => {
  ...
  return (
    <div>
      ...
      <PersonForm
        addName={addName}
        newName={newName}
        hdlNameChg={handleNameChange}
        hdlNumberChg={handleNumberChange}
        newNumber={newNumber}
      />
      <h2>Numbers</h2>
